...
def page_landing() -> None:
    render_header(
        ...
        "Text-Based Integrated Clinical Communication Rehearsal and Reflection",
        step_page="landing",
        show_restart=False,
    )
    ...
    with col_main:
        ...
        st.markdown(
            "This tool provides **text-based rehearsal** of clinical communication in fictional "
            "patient encounters. It trains **integrated communication**: building trust while "
            "gathering relevant clinical information, recognizing important concerns, and "
            "proposing safe next steps.\n\n"
            "In real clinical encounters, patients judge trustworthiness partly through perceived "
            "clinical competence. A student who only gives empathy without relevant clinical "
            "questions may still reduce trust. A student who asks the right questions but ignores "
            "emotion may also reduce trust. This tool trains both together."
        )
        st.markdown(f'<div class="clinical-task-box"><strong>Clinical framing.</strong> {CLINICAL_FRAMING}</div>', unsafe_allow_html=True)
        st.markdown("**What this tool can support:**")
        st.markdown(
            "- Rehearsing communication while gathering relevant history\n"
            "- Recognizing how clinical reasoning and emotional attunement interact\n"
            "- Practicing recognition of and response to patient emotional cues\n"
            "- Reflecting on the balance between warmth and clinical direction"
        )
        st.markdown("**What this tool cannot do:**")
        st.markdown(
            "- Assess diagnostic accuracy or clinical competence\n"
            "- Reproduce the difficulty of real clinical communication\n"
            "- Replace standardized patients, OSCEs, or faculty-observed encounters"
        )

    with col_side:
        st.markdown("#### Cases Available")
    ...
    st.markdown("#### Participant Information and Consent")
    with st.expander("Read before continuing", expanded=True):
        st.markdown(f"""
**Purpose.** This tool supports formative rehearsal of integrated clinical communication
through text-based interactions with fictional simulated patients.

**Fictional cases.** All patient cases are entirely fictional. Do not enter real patient
names, dates of birth, medical record numbers, or any identifying information.

**Formative feedback only.** Rubric scores are generated by a large language model.
They have not been validated as assessment instruments and should not be used for
formal evaluation or high-stakes decisions.

**Data.** Session data is stored locally only.
...
""")
    ...
    agreed = st.checkbox("I have read and understood the information above.")
...
def page_setup() -> None:
    ...
    with col_form:
        ...
        participant_id = st.text_input(
            ...
            placeholder="e.g., your student number or initials",
        )
        st.markdown("#### Training Level")
        training_level = st.selectbox(
            "Select your current training level",
            ["Pre-clinical (Year 1-2)", "Clinical (Year 3-4)", "Resident / Postgraduate", "Faculty / Observer"],
        )
    ...
    with col_info:
        ...
        st.markdown("""
**Guided Practice**
Hints available during the encounter. Cue interpretation shown directly.
Good for early learners or unfamiliar case types.

**Independent Practice**
No hints during encounter. Full formative feedback at completion.
Recommended for most learners.

**Faculty Review Mode**
For instructors previewing cases.
""")
        st.markdown(
...
overall agreement is 70–85%, limited by inherent classification ambiguity.""",
        ...
        (
            "Data collected",
            """Session data includes: participant ID, case ID, attempt number, full turn-by-turn
transcript, detected communication tags, patient state values at each turn, rubric scores,
pre/post self-efficacy ratings, rupture events, and reflection responses.

No real patient data is collected. All cases are fictional. Session data is stored locally.
Pilot use may involve exporting JSON/CSV for aggregate analysis.""",
        ),
        (
            "Suggested pilot use",
